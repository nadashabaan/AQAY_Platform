...
import SUC from "../assets/Images/SUC.png"; // Ensure path is correct
...
function SignUpFormC() {
  ...
  const handleRadioChange = (event) => {
    setFormData({
      ...formData,
      gender: event.target.value === "female", // Convert to boolean
    });
  };
  ...
  const handleSubmit = async (event) => {
    ...
    const payload = {
      ...
      password: formData.password,
      passwordConfirm: formData.passwordConfirm,
      gender: formData.gender, // Already boolean
      dateOfBirth: formData.dateOfBirth,
    };
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(
          errorData.message || "Signup failed. Please check your credentials."
        );
      }
    } catch (error) {
